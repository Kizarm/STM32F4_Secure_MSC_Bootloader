...
static DiskFile * CardInstance = 0;
...
void SysTick_Handler (void) {
  // asm volatile ("bkpt 0");
  if (CardInstance)
    CardInstance->disk_timerproc();
}

DiskFile::DiskFile() : spi(), block(512) {
  CardInstance = this;
}
...
bool DiskFile::Read (char * data, unsigned int block_no) {
  uint8_t token;
  unsigned offset = block_no;
  ...
  ok = 1;
  if (ok) spi.Recv ((uint8_t*)data, block);
...
//!//////////////////////////////////////////////////////////////////
uint8_t DiskFile::wait_ready (void) {
  uint8_t res;
...
void DiskFile::deselect (void) {
  spi.Select(false);
  spi.Read();
}
bool DiskFile::select (void) {
  spi.Select(true);
  if (wait_ready() != 0xFF) {
    deselect();
    return false;
...
bool DiskFile::rcvr_datablock (uint8_t * buff, uint32_t btr) {
  uint8_t token;
...
uint8_t DiskFile::send_cmd (uint8_t cmd, uint32_t arg) {
  uint8_t n, res;
  uint8_t com[6];
...
uint32_t DiskFile::get_disk_capacity (void) {
  uint8_t  n, csd[16];
  uint16_t  csize;
  uint32_t rc = 0;
...
void DiskFile::dmsDelay (int dms) {
  DiskTimerDelay = dms;
  while (DiskTimerDelay);
}
void DiskFile::disk_timerproc (void) {
  // dbg();
  uint8_t n;
