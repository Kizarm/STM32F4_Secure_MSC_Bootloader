...
static unsigned char stream_main_data_stat [MAD_BUFFER_MDLEN];
static mad_fixed_t   frame_overlap_stat    [2][32][18];
/*
 * NAME:	layer->III()
...
int mad_layer_III(struct mad_stream *stream, struct mad_frame *frame)
{
  ...
  if (stream->main_data == 0) {
    stream->main_data = & stream_main_data_stat; // mp3_malloc(MAD_BUFFER_MDLEN);
    /*
    if (stream->main_data == 0) {
  ...
  if (frame->overlap == 0) {
    frame->overlap = & frame_overlap_stat; // mp3_calloc(2 * 32 * 18, sizeof(mad_fixed_t));
    /*
    if (frame->overlap == 0) {
      stream->error = MAD_ERROR_NOMEM;
      return -1;
