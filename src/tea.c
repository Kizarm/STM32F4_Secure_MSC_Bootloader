...
// predpoklad je, ze blok je (len v bytech) delitelny 8
void encrypt_block (void * b, const uint32_t len, uint32_t const * const k) {
  uint32_t  count = len >> 2, i;
  uint32_t * data = (uint32_t*) b;
  for (i=0; i<count; i+=2) encrypt (data + i, k);
}
void decrypt_block (void * b, const uint32_t len, uint32_t const * const k) {
  uint32_t  count = len >> 2, i;
  uint32_t * data = (uint32_t*) b;
  for (i=0; i<count; i+=2) decrypt (data + i, k);
}
